        ...
        try:
            ...
            if artist_data and artist_data.get("images"):
                ...
                if images:
                    ...
                    log(f"    ✓ Found image from Spotify")
                    return image_url, config.IMAGE_SOURCE_SPOTIFY
        except Exception as e:
            log(f"    ✗ Spotify fetch failed: {e}")
    else:
        log(f"    → Spotify not configured (skipping)")

    log(f"    ✗ No image found from any source")
    return None, None
...
def _fetch_artist_profile(
    ...
    if not best or not best.get("id"):
        ...
        image_url, image_source = _fetch_artist_image_multi_source(artist_name, None, rate_limit_seconds, log_callback)
    ...
    log(f"    ✓ MusicBrainz profile found")

    # Fetch image from multiple sources
    image_url, image_source = _fetch_artist_image_multi_source(artist_name, artist, rate_limit_seconds, log_callback)
        ...
        try:
            ...
            if track_data and track_data.get("album"):
                ...
                if images:
                    # Get the highest quality image (first one)
                    image_url = images[0]["url"]
                    log(f"      ✓ Found cover from Spotify")
                    ...
                    try:
                        ...
                        log(f"      ✗ Failed to download Spotify image: {e}")
        except Exception as e:
            log(f"      ✗ Spotify search failed: {e}")

    log(f"      ✗ No cover art found from any source")
...
def _store_artist_profile(
    ...
    profile, image_url, image_source = _fetch_artist_profile(artist.name, rate_limit_seconds, log_callback)
    ...
    image_id = None
    if image_url:
        log(f"    → Downloading image from {actual_source}...")
        try:
            image_bytes, mime_type = _download_binary(
                image_url,
                config.IMAGE_REQUEST_TIMEOUT_SEC,
    ...
    with with_image_connection() as image_conn:
        with image_conn.cursor() as image_cur:
            for song in songs:
                ...
                log(f"[Songs {songs_processed}/{total_songs}] Processing: {artist_display} - {song.title}")
                ...
                try:
                    ...
                    if release_id and release_id not in synced_releases:
                        ...
                        log(f"  → Syncing album metadata: {release_title or song.album or 'Unknown Album'}")
                        try:
                            ...
                            log(f"    ✗ Album metadata failed: {str(e)}")
                            failed += 1
                    ...
                    if song_delta > 0 or album_delta > 0:
                        log(f"  ✓ Cover art stored (song: {song_delta}, album: {album_delta})")
                    else:
            ...
            for artist in artists:
                ...
                try:
                    ...
                    if profile_delta > 0:
                        if image_delta > 0:
                            log(f"  ✓ Profile and image stored")
                        else:
                            log(f"  ✓ Profile stored (no image available)")
                    else:
