...
from hash_pipeline.pipeline import output_path_for_wav, save_normalized_wav


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Normalize PCM WAV files for hashing.",
    )
    parser.add_argument("input", help="WAV file or directory of WAV files.")
    parser.add_argument("output", help="Output file or directory for normalized WAVs.")
    parser.add_argument(
        "--overwrite",
        action="store_true",
    ...
    parser.add_argument(
        "--write-metadata",
        action="store_true",
        help="Write a JSON sidecar with normalization metadata.",
    )
...
def main() -> int:
    ...
    for idx, wav_file in enumerate(wav_files, start=1):
        ...
        try:
            ...
            print(f"[{idx}/{total}] normalized: {target_path}")
